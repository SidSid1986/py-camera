...
class MainWindow(QtWidgets.QMainWindow):
        ...
        while not self.imgThreadExit.is_set() and self.hScanner:
            try:
                ...
                if dataLength > 0:
                    ...
                    # 调试输出前几个点，确保数据不是全0
                    # print("[DEBUG] 采集点数:", dataLength)
                    # print("[DEBUG] X前10:", [self.dScannerBufferX[i] for i in range(min(5, dataLength))])
                    # print("[DEBUG] Z前10:", [self.dScannerBufferZ[i] for i in range(min(5, dataLength))])
                    # print("[DEBUG] I前10:", [self.iScannerBufferI[i] for i in range(min(5, dataLength))])
    ...
    def draw_profile(self):
        ...
        if self.iLengthReceivedData > 1:
            ...
            # 1. 取C++ setImageParameters后参数
            X_RANGE_AT_END = float(self.strXRangeAtEnd)
            Z_START = float(self.strZstart)
            Z_RANGE = float(self.strZrange)

            # 2. 计算缩放
            scaleX = w / X_RANGE_AT_END
            scaleY = h / Z_RANGE
            ...
            for i in range(self.iLengthReceivedData):
                x = int(w - scaleX * (x_vals[i] + X_RANGE_AT_END / 2))
                if (z_vals[i] - Z_START) > 0:
                    z = int(h - scaleY * (z_vals[i] - Z_START))
                    # 边界保护
                    if 0 <= x < w and 0 <= z < h:
            ...
            for i in range(self.iLengthReceivedData):
                x = int(w - scaleX * (x_vals[i] + X_RANGE_AT_END / 2))
                # 1023 是C++里的最大强度，Python也用这个
                z = int(h * (1 - i_vals[i] / 1023))
                if 0 <= x < w and 0 <= z < h:
                    painter.drawPoint(x, z)
    ...
    def on_screenshot(self):
        """
        1. 获取labelImage内的pixmap，如果没有图片，弹窗警告。
        2. 复制一份pixmap，防止原对象被多线程破坏。
        3. 弹出文件对话框，让用户选择保存路径。
        4. 保存为PNG图片，并弹窗提示是否成功。
        5. 全过程异常捕获，保证不闪退。
        """
        try:
            # 1. 检查labelImage对象
    ...
    def on_timer(self):
        if self.hScanner:
            ...
            # print(f"[定时器] 当前频率: {freq} Hz, 上次采集长度: {self.iLengthReceivedData}")
        else:
